...
function geodir_advance_search_more_filters() {
    ...
    <div class="geodir-filter-container">
        <div class="geodir-more-filters customize_filter-in clearfix <?php echo 'gd-filter-' . $stype;?>" <?php echo $style;?>>
...
function geodir_search_show_searched_params( $post_type ) {
	...
	if ( ! empty( $_REQUEST['snear'] ) ) {
		$params[] = '<label class="gd-adv-search-label gd-adv-search-near"><i class="fas fa-times" aria-hidden="true"></i> ' . sanitize_text_field( $_REQUEST['snear'] ) . '</label>';
	}
			...
			switch( $field->input_type ) {
				case 'RANGE': {
					switch( $field->search_condition ) {
						case 'SINGLE': {
							if ( isset( $_REQUEST['s' . $htmlvar_name] ) && $_REQUEST['s' . $htmlvar_name] !== '' ) {
								$params[] = '<label class="gd-adv-search-label gd-adv-search-range gd-adv-search-' . $htmlvar_name . '" data-name="s' . $htmlvar_name . '"><i class="fas fa-times" aria-hidden="true"></i> ' . sanitize_text_field( $_REQUEST['s' . $htmlvar_name] ) . '</label>';
							}
						}
						break;
						case 'FROM': {
							...
							if ( $this_search != '' ) {
								...
								$params[] = '<label class="gd-adv-search-label gd-adv-search-range gd-adv-search-' . $htmlvar_name . '" ' . $extra_attrs . '><i class="fas fa-times" aria-hidden="true"></i> <label class="gd-adv-search-label-t">' . $frontend_title . ': </label>' . $this_search . '</label>';
							}
						}		
						break;
						case 'RADIO': {
							...
							if (isset( $_REQUEST[ $htmlvar_name ] ) && $_REQUEST[ $htmlvar_name ] !== '' ) {
								...
								$params[] = '<label class="gd-adv-search-label gd-adv-search-range gd-adv-search-' . $field->htmlvar_name . '" ' . $extra_attrs . '><i class="fas fa-times" aria-hidden="true"></i> '.__( 'Within', 'geodiradvancesearch' ) . ' ' . (int)$_REQUEST[ $htmlvar_name ].' '.__( $uom, 'geodirectory' ) . '</label>';
							}
						}
						break;
						default : {
				...
				case 'DATE': {
					if ( ! empty( $_REQUEST[ $htmlvar_name ] ) && ( $field->data_type == 'DATE' || $field->data_type == 'TIME' ) ) {
						...
						if ( $filters != '' ) {
							$params[] = '<label class="gd-adv-search-label gd-adv-search-date gd-adv-search-' . $htmlvar_name . '" ' . $extra_attrs . '><i class="fas fa-times" aria-hidden="true"></i> <label class="gd-adv-search-label-t">' . $frontend_title . ': </label>' . $filters . '</label>';
						}
					}
				}
				break;
				default: {
					if (isset( $_REQUEST['s' . $htmlvar_name] ) && ( ( is_array( $_REQUEST['s' . $htmlvar_name] ) && !empty( $_REQUEST['s' . $htmlvar_name] ) ) || ( ! is_array(  $_REQUEST['s' . $htmlvar_name] ) && $_REQUEST['s' . $htmlvar_name] !== '' ) ) ) {
						if ( is_array( $_REQUEST['s' . $htmlvar_name] ) ) {
							...
							if ( $value ) {
								$value = '<label class="gd-adv-search-label-t">' . $frontend_title . ': </label>' . $value;
							}
						} else {
						...
						if ( $value != '' ) {
							$params[] = '<label class="gd-adv-search-label gd-adv-search-default gd-adv-search-' . $htmlvar_name . '" ' . $extra_attrs . '><i class="fas fa-times" aria-hidden="true"></i> ' . $value . '</label>';
						}
					}
				}
				break;
    ...
    if ( ! empty( $params ) ) {
		$searched_params = ! empty( $params ) ? '<div class="gd-adv-search-labels">' . implode( '', $params ) . '</div>' : '';
	} else {
		$searched_params = '';
	}
