...
class GeoDir_Adv_Search_Business_Hours {
	...
	public static function business_hours_options( $placeholder = false ) {
		...
		if ( $placeholder ) {
			$options = array_merge( array( '' => __( 'Open Hours', 'geodiradvancesearch' ) ), $options );
		}
	...
	public static function save_post_business_hours( $post_id, $value = NULL, $country = '' ) {
		...
		if ( ! empty( $rows ) ) {
			...
			foreach ( $rows as $row ) {
				$sql_parts[] = "(" . (int) $post_id . ", " . (int) $row['open'] . ", " . (int) $row['close'] . ", " . (int) $row['open_utc'] . ", " . (int) $row['close_utc'] . ", " . (int) $row['open_dst'] . ", " . (int) $row['close_dst'] . ", '" . $timezone_string . "', " . (int) $has_dst . ", " . (int) $is_dst . ")";
			}

			$return = $wpdb->query( "INSERT INTO `" . GEODIR_BUSINESS_HOURS_TABLE . "` (`post_id`, `open`, `close`, `open_utc`, `close_utc`, `open_dst`, `close_dst`, `timezone_string`, `has_dst`, `is_dst`) VALUES " . implode( ', ', $sql_parts ) );
	...
	public static function business_hours_condition( $filter = 'now', $alias = NULL, $operator = 'AND' ) {
		...
		if ( in_array( $filter, $days ) ) {
			...
			$condition = "( ( {$open} <= {$alias}open AND {$alias}open <= {$close} ) OR ( {$open} <= {$alias}close AND {$alias}close <= {$close} ) )";
		} elseif ( $filter == 'weekend' ) {
			$condition .= "( ( ";
			$sat_open = geodir_hhmm_to_bh_minutes( '00:00', 6 );
			$sat_close = $sat_open + ( 24 * 60 ) - 1;

			$condition .= "( {$sat_open} <= {$alias}open AND {$alias}open <= {$sat_close} ) OR ( {$sat_open} <= {$alias}close AND {$alias}close <= {$sat_close} )";
			...
			$condition .= "( {$sun_open} <= {$alias}open AND {$alias}open <= {$sun_close} ) OR ( {$sun_open} <= {$alias}close AND {$alias}close <= {$sun_close} )";
	...
	public static function cpt_merge_business_hours( $post_type, $force = false ) {
		...
		if ( $force ) {
			...
			$results = $wpdb->get_results( "SELECT post_id, business_hours{$country_field} FROM `{$table}` WHERE `business_hours` != '' AND `business_hours` IS NOT NULL ORDER BY `post_id` ASC" );
		} else {
			$country_field = GeoDir_Post_types::supports( $post_type, 'location' ) ? ", pd.country" : "";
			$results = $wpdb->get_results( "SELECT DISTINCT pd.post_id, pd.business_hours{$country_field} FROM `{$table}` AS pd LEFT JOIN `" . GEODIR_BUSINESS_HOURS_TABLE . "` AS bh ON bh.post_id = pd.post_id WHERE pd.`business_hours` != '' AND pd.`business_hours` IS NOT NULL AND bh.post_id IS NULL ORDER BY pd.`post_id` ASC" );
		}
	...
	public static function tool_merge_business_hours( $tools ) {
		...
		if ( ! empty( $post_types ) ) {
			foreach ( $post_types as $post_type ) {
				...
				if ( $force ) {
					$results = $wpdb->get_results( "SELECT post_id, business_hours FROM `{$table}` WHERE `business_hours` != '' AND `business_hours` IS NOT NULL ORDER BY `post_id` ASC" );
				} else {
					$results = $wpdb->get_results( "SELECT DISTINCT pd.post_id, pd.business_hours FROM `{$table}` AS pd LEFT JOIN `" . GEODIR_BUSINESS_HOURS_TABLE . "` AS bh ON bh.post_id = pd.post_id WHERE pd.`business_hours` != '' AND pd.`business_hours` IS NOT NULL AND bh.post_id IS NULL ORDER BY pd.`post_id` ASC" );
				}
