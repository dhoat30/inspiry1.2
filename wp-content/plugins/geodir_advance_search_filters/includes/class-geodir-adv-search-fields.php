...
//			$html .= str_replace( array( '<li>', '</li>' ), '', geodir_advance_search_options_output( $options, $cf, $post_type, __( stripslashes( $cf->frontend_title ), 'geodirectory' ) ) );
	...
	public static function output_field_distance( $html, $field_info, $post_type ) {
		...
		$field_class = 'gd-search-li-' . (int) $has_fieldset . ' gd-search-li-' . $field_info->htmlvar_name;
		...
		if ( $field_info->search_condition == "RADIO" ) {
			...
			if ( $field_info->htmlvar_name == 'distance' && $field_info->extra_fields != '' ) {
				...
				$display_label = $has_fieldset ? '<label for="geodir_search_' . $field_info->htmlvar_name . '">' . $field_label . '</label>' : '';
				...
				if ( $sort_options != '' ) {
					echo '<ul><li class="' . $field_class . '">' . $display_label . '<select id="" class="cat_select" name="sort_by">';
