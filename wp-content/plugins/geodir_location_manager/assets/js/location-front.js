jQuery(document).ready(function() {
    ...
    // setup the trigger for the location switcher opener
    ...
    setTimeout(function() { // add small delay incase mobile menu is created on the fly. 100 shoudl do it
        var keyup_timer;
        var gdlm_msgWait = geodir_location_params.LOCATION_PLEASE_WAIT;
        var gdlm_ajax_url = geodir_location_params.ajax_url;
        ...
        if (!geodir_lm_chosen_supported()) {
            ...
            if (jQuery("select.geodir_location_switcher_chosen").length > 0) {
                jQuery("select.geodir_location_switcher_chosen").chosen({
                    no_results_text: geodir_location_params.LOCATION_CHOSEN_NO_RESULT_TEXT
                });
            }
        ...
        if (!geodir_lm_chosen_supported()) {
            ...
            jQuery(document).find('.geodir_location_sugestion select[name="gd_location"] option').click(function() {
                if (jQuery(this).attr('selected') == 'selected' || jQuery(this).attr('value') == geodir_location_params.gd_base_location) {
        ...
        jQuery(document).on("click", ".gd-loc-loadmore button", function(e) {
            ...
            var $obj = $this.closest('.geodir-locListing_column'),
                $mainObj = $obj.closest('.geodir_locListing_main');
            var type = $obj.data('type'),
                total = parseInt($obj.data('limit')),
    ...
    jQuery("select.geodir_location_switcher_chosen").each(function() {
        ...
        if (curr_chosen.data('ajaxchosen') == '1' || curr_chosen.data('ajaxchosen') === undefined) {
            ...
            curr_chosen.ajaxChosen({
                keepTypingMsg: geodir_location_params.LOCATION_CHOSEN_KEEP_TYPE_TEXT,
                lookingForMsg: geodir_location_params.LOCATION_CHOSEN_LOOKING_FOR_TEXT,
                type: 'GET',
...
jQuery(document).ready(function() {
    ...
    jQuery('select.geodir_location_add_listing_chosen').bind('change', function() {
        ...
        if (location_type == 'region') {
            ...
            if (!geodir_lm_chosen_supported() && gd_region_div.find("#post_region_new").attr('id')) {
                if (region_val) {
                    jQuery('.gd-new-loc-div', gd_region_div).hide();
                } else {
                    jQuery('.gd-new-loc-div', gd_region_div).show();
                }
            }
        ...
        if (location_type == 'city') {
            ...
            if (!geodir_lm_chosen_supported() && gd_city_div.find("#post_city_new").attr('id')) {
                if (city_val) {
                    jQuery('.gd-new-loc-div', gd_city_div).hide();
                } else {
                    jQuery('.gd-new-loc-div', gd_city_div).show();
                }
            }
...
function geodir_no_chosen_add_search(cont, tab) {
    ...
    if (tab == 'city') {
        placeHold = geodir_location_params.gd_text_search_city;
    } else if (tab == 'region') {
        placeHold = geodir_location_params.gd_text_search_region;
    } else if (tab == 'country') {
        placeHold = geodir_location_params.gd_text_search_country;
    }
    placeHold = placeHold != '' ? placeHold : geodir_location_params.gd_text_search_location;
...
function gdShareLocationError(error) {
    switch (error.code) {
        case error.PERMISSION_DENIED:
            alert(geodir_location_params.PERMISSION_DENINED);
            break;
        case error.POSITION_UNAVAILABLE:
            alert(geodir_location_params.POSITION_UNAVAILABLE);
            break;
        case error.TIMEOUT:
            alert(geodir_location_params.DEFAUTL_ERROR);
            break;
        case error.UNKNOWN_ERROR:
            alert(geodir_location_params.UNKNOWN_ERROR);
            break;
    }
}
...
function gdLocationSetupUserLoc() {
    if (my_location) {
        ...
        jQuery('.snear').val(geodir_location_params.msg_Near + ' ' + geodir_location_params.msg_Me);
        ...
        if (lat && lon) {
            jQuery('.geodir-search .fa-compass').css("color", "#087CC9");
            jQuery('.gt_near_me_s').prop('checked', true);
            jQuery('.snear').val(geodir_location_params.msg_Near + ' ' + geodir_location_params.msg_User_defined);
            jQuery('.sgeo_lat').val(lat);
            jQuery('.sgeo_lon').val(lon);
        } else if (jQuery('.snear').length && jQuery('.snear').val().match("^" + geodir_location_params.msg_Near)) {
