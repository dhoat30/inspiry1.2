...
class GeoDir_Location_SEO {
		...
		if ( $gd_page == 'location' ) {
			...
			if ( ! empty( $type ) && ! empty( $value ) ) {
				...
				if ( ! empty( $seo ) && ! empty( $seo->meta_title ) ) {
					$title = __( $seo->meta_title, 'geodirlocation' );
				}
			}
			...
			if ( ! empty( $gd_neighbourhood ) && GeoDir_Location_Neighbourhood::is_active() ) {
				...
				if ( ! empty( $hood ) && ( ! empty( $hood->neighbourhood ) || ! empty( $hood->meta_title ) ) ) {
					$meta_title = ! empty( $hood->meta_title ) ? __( $hood->meta_title, 'geodirlocation' ) : $hood->neighbourhood;
	...
	public static function filter_meta_desc( $meta_desc ) {
		...
		if ( ! empty( $type ) && ! empty( $value ) ) {
			...
			if ( ! empty( $seo ) && ! empty( $seo->meta_desc ) ) {
				$meta_desc = __( $seo->meta_desc, 'geodirlocation' );
			}
		}
		...
		if ( ! empty( $gd_neighbourhood ) && GeoDir_Location_Neighbourhood::is_active() ) {
			...
			if ( ! empty( $neighbourhood ) && ! empty( $neighbourhood->meta_description ) ) {
				$meta_desc = strip_tags( stripslashes( __( $neighbourhood->meta_description, 'geodirlocation' ) ) );
			}
		}
	...
	public static function get_image( $type = 'city', $slug, $country_slug = '', $region_slug = '', $size = 'full', $icon = false, $attr = '' ) {
		global $wpdb;
	...
	public static function get_image_src( $type = 'city', $slug, $country_slug = '', $region_slug = '', $size = 'full', $icon = false ) {
		global $wpdb;
	...
	public static function wpseo_sitemap_init() {
		...
		global $wpseo_sitemaps, $gd_wpseo_options, $gd_wpseo_timezone, $gd_wpseo_date_helper, $gd_wpseo_max_entries, $gd_post_types,$geodirectory;
	...
	public static function wpseo_location_sitemap_index( $location_type ) {
		global $gd_wpseo_timezone, $gd_wpseo_max_entries, $gd_wpseo_index, $gd_post_types, $gd_wpseo_date_helper;
	...
	public static function wpseo_post_type_sitemap_index( $location_type ) {
		global $gd_sitemap_global, $gd_wpseo_timezone, $gd_wpseo_max_entries,$gd_wpseo_date_helper, $gd_wpseo_index, $gd_post_types;
	...
	public static function wpseo_post_type_sitemap_content( $location_type, $post_type ) {
		global $wp, $geodirectory, $gd_post_type_archive_links, $gd_wpseo_date_helper, $wpseo_sitemaps, $gd_wpseo_options, $gd_wpseo_timezone, $gd_wpseo_max_entries, $gd_post_types;
	...
	public static function wpseo_taxonomy_sitemap_index( $location_type ) {
		global $gd_sitemap_global, $gd_wpseo_timezone, $gd_wpseo_date_helper, $gd_wpseo_max_entries, $gd_wpseo_index;
	...
	public static function wpseo_taxonomy_sitemap_content($location_type, $taxonomy) {
		global $wp, $wpseo_sitemaps, $geodirectory, $gd_wpseo_date_helper, $gd_wpseo_options, $gd_wpseo_timezone, $gd_wpseo_max_entries;
	...
	public static function wpseo_location_sitemap_content( $location_type ) {
		global $wpseo_sitemaps, $gd_wpseo_options,$gd_wpseo_date_helper,  $gd_wpseo_timezone, $gd_wpseo_max_entries, $gd_post_types;
	...
	public static function wpseo_sitemap_get_locations_post_types( $location_type, $gd_post_types, $page = false ) {
		...
		foreach ( $gd_post_types as $gd_post_type ) {
			...
			$query = "SELECT DISTINCT " . $group_by . " AS u, MAX(p.post_modified_gmt) AS date_gmt, " . $fields . " FROM " . GEODIR_LOCATIONS_TABLE . " l LEFT JOIN " . $detail_table . " pd ON {$join_condition} LEFT JOIN " . $wpdb->posts . " p ON p.ID = pd.post_id WHERE p.post_type = '" . $gd_post_type . "' AND p.post_status = 'publish' AND l.location_id IS NOT NULL AND p.ID IS NOT NULL " . $where . " GROUP BY u ORDER BY date_gmt DESC";
	...
	public static function wpseo_sitemap_get_locations_taxonomies( $location_type, $gd_post_type, $taxonomy_type, $page = false ) {
		...
		switch ($location_type) {
			...
			case 'country':
				$fields = "l.country_slug AS country";
				$join_condition = "pd.country = l.country";
		...
		$query = "SELECT tt.term_id, MAX(p.post_modified_gmt) AS date_gmt, " . $fields . " FROM " . GEODIR_LOCATIONS_TABLE . " l LEFT JOIN " . $detail_table . " pd ON {$join_condition} LEFT JOIN " . $wpdb->posts . " p ON p.ID = pd.post_id LEFT JOIN `" . $wpdb->term_relationships . "` AS tr ON tr.object_id = p.ID LEFT JOIN `" . $wpdb->term_taxonomy . "` AS tt ON ( tt.term_taxonomy_id = tr.term_taxonomy_id AND tt.taxonomy = '" . $taxonomy . "' ) WHERE p.post_type = '" . $gd_post_type . "' AND p.post_status = 'publish' AND l.location_id IS NOT NULL AND tt.term_id IS NOT NULL " . $where . " GROUP BY " . $group_by . " ORDER BY date_gmt DESC";
		...
		if ( GeoDir_Post_types::supports( $post_type, 'location' ) && ( $seo = self::get_location_seo() ) ) {
			if ( ! empty( $seo ) && ! empty( $seo->cpt_desc ) ) {
				...
				if ( ! empty( $cpt_desc ) && is_array( $cpt_desc ) && ! empty( $cpt_desc[ $post_type ] ) ) {
					...
					if ( $_description != '' ) {
						$description = __( $_description, 'description' );
					}
				}
			}
		}
