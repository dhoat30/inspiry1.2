...
class GeoDir_Location_API {
	...
	public static function get_locations( $params = array() ) {
		...
		if ( ! empty( $args['country'] ) ) {
			$where[] = "country_slug LIKE '" . wp_slash( $args['country'] ) . "'";
		}
		if ( ! empty( $args['region'] ) ) {
			$where[] = "region_slug LIKE '" . wp_slash( $args['region'] ) . "'";
		}
		if ( ! empty( $args['city'] ) ) {
			$where[] = "city_slug LIKE '" . wp_slash( $args['city'] ) . "'";
		}
	...
	public static function get_location( $params = array() ) {
		...
		if ( $args['what'] == 'city' && ! empty( $args['id'] ) ) {
			...
			if ( ! empty( $args['country'] ) ) {
				$where[] = "( country_slug LIKE '" . wp_slash( $args['country'] ) . "' OR country_slug = '" . wp_slash( $args['country'] ) . "' )";
			}
			
			if ( ! empty( $args['region'] ) ) {
				$where[] = "( region_slug LIKE '" . wp_slash( $args['region'] ) . "' OR region_slug = '" . wp_slash( $args['region'] ) . "' )";
			}
			
			if ( ! empty( $args['city'] ) ) {
				$where[] = "( city_slug LIKE '" . wp_slash( $args['city'] ) . "' OR city_slug = '" . wp_slash( $args['city'] ) . "' )";
			}
		}
	...
	public static function get_neighbourhoods( $params = array() ) {
		...
		if ( ! empty( $args['location_id'] ) ) {
			$where[] = "h.hood_location_id LIKE '" . wp_slash( $args['location_id'] ) . "'";
		}
		if ( ! empty( $args['country'] ) ) {
			$where[] = "l.country_slug LIKE '" . wp_slash( $args['country'] ) . "'";
		}
		if ( ! empty( $args['region'] ) ) {
			$where[] = "l.region_slug LIKE '" . wp_slash( $args['region'] ) . "'";
		}
		if ( ! empty( $args['city'] ) ) {
			$where[] = "l.city_slug LIKE '" . wp_slash( $args['city'] ) . "'";
		}
